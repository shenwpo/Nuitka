...
ITERATOR_NEXT(PyThreadState *tstate, struct Nuitka_ExceptionPreservationItem *exception_state, PyObject *iterator) {
    CHECK_OBJECT(iterator);
    ...
    if (unlikely(iternext == NULL)) {
        ...
        SET_EXCEPTION_PRESERVATION_STATE_FROM_TYPE0_FORMAT1(tstate, exception_state, PyExc_TypeError,
                                                            "%s object is not an iterator", Py_TYPE(iterator)->tp_name);
#else
        SET_EXCEPTION_PRESERVATION_STATE_FROM_TYPE0_FORMAT1(
            tstate, exception_state, PyExc_TypeError, "'%s' object is not an iterator", Py_TYPE(iterator)->tp_name);
#endif
